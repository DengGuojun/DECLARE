...
public class DeclareInfoDao {
	...
	public PageResultBean<DeclareInfoBean> getDeclareInfoPageListByMap(HashMap<String, String> condMap,
		...
		try {
			...
			result = dbExecutor
					.getPageResult(sql, orderQuery, condList, paramList, DeclareInfoBean.class, pageBean, db);
		} catch (Exception e) {
			log.error("", e);
		} finally {
			try {
	...
	public DeclareInfoBean getDeclareInfoByCondition(int userId, int declareType, String declareYear) {
		...
		try {
			db = dbFactory.getDBObjectR();
			String sql = "select * from declare_info where user_id=? and declare_type=? and declare_year=? and status=?";

			PreparedStatement ps = db.getPreparedStatement(sql);
			ps.setInt(1, userId);
			ps.setInt(2, declareType);
			ps.setString(3, declareYear);
			ps.setInt(4, Constants.STATUS_VALID);
