...
@WebServlet("/declare/IndustryTypeList.do")
public class IndustryTypeList extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		PageResultBean<IndustryTypeBean> result = business.getIndustryTypePageListByMap(condMap, pageBean);

		// 页面数据绑定
		request.setAttribute("IndustryTypeList", result.getRecordList());
		pageBean.init(pageNum, pageSize, result.getTotalRecordNumber());
		request.setAttribute("PageResult", pageBean);
		request.setAttribute("CondList", MapKit.map2List(condMap));
		request.setAttribute("AdminUserHelper", adminUserHelper);
		// 页面转发
		RequestDispatcher rd = request.getRequestDispatcher(DeclareConsoleConfig.PAGE_PATH + "IndustryTypeList.jsp");
