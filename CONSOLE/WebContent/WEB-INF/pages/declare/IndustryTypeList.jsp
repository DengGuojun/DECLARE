...
<%
	...
	List<IndustryTypeBean> list = (List<IndustryTypeBean>)request.getAttribute("IndustryTypeList");
...
<p class="article_tit">产业类型列表</p>
<form name="formSearch" method="post" action="IndustryTypeList.do">
<div class="search_form">
  	<em class="em1">产业类型名称：</em>
  ...
  <table width="100%" border="0"  cellpadding="0" class="table_style">
    ...
    <%
    for(IndustryTypeBean bean:list){%> 
    <tr>
      <td><%=bean.getTypeId() %></td>
      <td><%=bean.getTypeName() %></td>
      <td><%=Constants.STATUS_MAP.get(bean.getStatus())%></td>
      <td align="center">
      	<a href="/declare/IndustryTypeManage.do?typeId=<%=bean.getTypeId()%>&readOnly=true">查看</a> 
      	<%if(adminUserHelper.hasPermission(DeclareInfoResource.DECLARE_INFO, OperationConfig.UPDATE)){ %>
       |<a href="/declare/IndustryTypeManage.do?typeId=<%=bean.getTypeId()%>&readOnly=false">修改</a>
       <%} %>
      </td>
    </tr>	
    <%} %>
...
<%if(adminUserHelper.hasPermission(DeclareInfoResource.DECLARE_INFO, OperationConfig.CREATE)){ %>
  	<input type="button" name="button" id="button" value="新建" onclick="javascript:location.href='IndustryTypeManage.do'">
<%} %>   	
</li>
