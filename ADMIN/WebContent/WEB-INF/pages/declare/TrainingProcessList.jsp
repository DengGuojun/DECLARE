...
<link href="<%=STATIC_URL %>/css/common_new_new.css" type="text/css"
	rel="stylesheet">
<link href="<%=STATIC_URL %>/css/common_new.css" type="text/css"
	rel="stylesheet">
<link href="<%=STATIC_URL %>/css/common.css" type="text/css"
	rel="stylesheet">
<link href="<%=STATIC_URL %>/css/main.css" type="text/css"
	rel="stylesheet">
<link href="<%=STATIC_URL %>/css/SlideMenuStyle.css" type="text/css"
	rel="Stylesheet">
<link href="<%=STATIC_URL %>/css/default.css" type="text/css"
	rel="stylesheet">
<link href="<%=STATIC_URL %>/css/StyleBlue.css" type="text/css"
	rel="Stylesheet">
<link href="<%=STATIC_URL %>/css/index_main.css" type="text/css"
	rel="Stylesheet">
<link href="<%=STATIC_URL %>/css/tab_select.css" type="text/css"
	rel="Stylesheet">
...
<body class="body-index">
						...
						<div class="detail_right">
							<div class="content_wrap fixed_height">
								<div class="detail_right_title h1 white">
									<span style="float: left;">培训进展</span>
								</div>

								<div class="right_bg" style="padding-left: 5px;">
									<form name="formSearch" method="post"
										action="TrainingProcessList.do">
										<input type="hidden" name="declareType" id="declareType"
											value="<%=declareType %>" />
										<div class="form-group form-horizontal">
											<label>年份：</label> <select class="form-control"
												name="declareYear" id="declareYear">
												<%for(Integer InfoYear : declareInfoHelper.getTrainingYearList()){ %>
												<option value="<%=InfoYear %>"
													<%=ParamKit.getIntParameter(request, "declareYear", 0) == InfoYear? "selected" : ""%>><%=InfoYear %></option>
												<%}%>
											</select>
										</div>
										<input type="hidden" id="queryProvince" name="queryProvince"
											value="<%=queryProvince !=null ? queryProvince : "" %>" /> <input
											type="hidden" id="queryCity" name="queryCity"
											value="<%=queryCity !=null ? queryCity : "" %>" /> <input
											type="hidden" id="queryRegion" name="queryRegion"
											value="<%=queryRegion !=null ? queryRegion : "" %>" />
										<div class="form-group form-horizontal">
											<label>地区：</label> <select name="selectProvince"
												id="selectProvince" onchange="showCity()">
												<option value="">请选择</option>
											</select> <select name="selectCity" id="selectCity"
												onchange="showRegion()">
												<option value="">请选择</option>
											</select> <select name="selectRegion" id="selectRegion"
												onchange="setRegion()">
												<option value="">请选择</option>
											</select>
										</div>
										<div class="form-group form-horizontal">
											<label>数据状态：</label> <select class="form-control"
												name="dataStatus" id="dataStatus">
												<option value="">全部</option>
												<% for(StatusBean<String, String> statusBean:DeclareInfoRecommendConfig.COUNT_STATUS_LIST){ %>
												<option value="<%=statusBean.getStatus() %>"
													<%=(statusBean.getStatus().equals(dataStatus))?"selected":"" %>><%=statusBean.getValue() %></option>
												<%} %>
											</select>
										</div>
									...
									<%if(declareType == 0){ %>
									<div>
										<table class="gv js-gvHead" cellspacing="0" rules="all"
											border="1" style="border-collapse: collapse;">
											<tbody>
												<tr class="gv_Head">
													<th rowspan="2" style="width: 30px;">序号</th>
													<th rowspan="2">地区</th>
													<th rowspan="2">项目县数（个）</th>
													<th rowspan="2">任务总数（个）</th>
													<th rowspan="2">已完成任务数（个）</th>
													<th rowspan="2">已完成比例</th>
													<th colspan="3">现代青年农场主</th>
													<th colspan="3">新型农业经营主体带头人</th>
													<th colspan="3">生产经营型职业农民</th>
													<th colspan="3">专业技能型职业农民</th>
													<th colspan="3">专业服务型职业农民</th>
												</tr>
												<tr class="gv_Head">
													<th>任务数</th>
													<th>完成数</th>
												...
												<tr>
													...
													<td><a
														href="TrainingProcessList.do?queryProvince=<%=key %>&dataStatus=<%=dataStatus%>&declareYear=<%=declareYear%>"><%=key %></a></td>
													<%}else if(declareReportMap.get(key).get(0).equals(DeclareAdminConfig.ADMIN_LEVEL_CITY)){ %>
													<td><a
														href="TrainingProcessList.do?queryCity=<%=key %>&queryProvince=<%=declareReportMap.get(key).get(20) %>&dataStatus=<%=dataStatus%>&declareYear=<%=declareYear%>"><%=key %></a></td>
													<%} else if(declareReportMap.get(key).get(0).equals(DeclareAdminConfig.ADMIN_LEVEL_REGION)){ %>
													<td><a
														href="TrainingProcessList.do?queryRegion=<%=key %>&queryProvince=<%=declareReportMap.get(key).get(20) %>&queryCity=<%=declareReportMap.get(key).get(21) %>&dataStatus=<%=dataStatus%>&declareYear=<%=declareYear%>"><%=key %></a></td>
													<%} else {%>
													<td><%=key %></td>
													<%}%>
	...
	<script src="<%=STATIC_URL%>/js/jquery-1.8.3.min.js"
	   	...
	   	for(var i =0;i<items.length;i++) {
	      	...
	      	var queryRegion = "<%=queryRegion%>";
				if (fixRegion == item.regionName) {
					sel.append("<option value = '"+item.regionId+"' selected>"
					...
					sel.append("<option value = '"+item.regionId+"'>"
							+ item.regionName + "</option>");
				}
			}
			;
