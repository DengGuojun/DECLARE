...
<%
	...
	String model = ParamKit.getParameter(request, "model", "");
	Integer trainingType = ParamKit.getIntParameter(request, "trainingType", 0);
                         ...
                         <td valign="top">
                            <div class="detail_right">
                                <div class="content_wrap fixed_height">
                            		...
                            		<%}else{ %>
                            			<span id="ctl00_ContentPlaceHolder1_Label1" style="float: left;">各地机构类型统计</span>
                            	     <%} %>                                       
                                    </div>
                                    <div class="con fl pt_10">
                                        <ul id="tags" class="tags" style="width: 1100px;">
                            				...
                            				<li>
                            				<%} %> 
                                                <a class="text_15" href="TrainingOrganizationCountList.do?organizationType=<%=TrainingOrganizationConfig.ORGANIZATION_TRAINING %>&model=<%=model%>">培训机构</a>
                                            </li>
                            				...
                            				<li>
                            				<%} %> 
                                                <a class="text_15" href="TrainingOrganizationCountList.do?organizationType=<%=TrainingOrganizationConfig.ORGANIZATION_AUTHORIZEDE %>&model=<%=model%>">认定机构</a>
                                            </li>
                                        </ul>
                                        <div id="tagContent">
                                            <div class="tagContent selectTag scroller-box">
                                                <div class="right_bg pl_5">
                                                <form name="formSearch" method="post" action="TrainingOrganizationCountList.do">
                                                        ...
                                                        <table class="gv js-gvHead" cellspacing="0" rules="all" border="1" style="border-collapse:collapse;">
                                                            <tbody>
                                                                ...
                                                                <tr>
                            										...
                            										<td><a href="TrainingOrganizationCountList.do?queryProvince=<%=key %>&organizationType=<%=organizationType %>&model=<%=model %>&trainingType=<%=trainingType%>&trainingYear=<%=trainingYear%>"><%=key %></a></td>
                            										<%}else if(declareReportMap.get(key).get(0).equals(DeclareAdminConfig.ADMIN_LEVEL_CITY)){ %>
                            										<td><a href="TrainingOrganizationCountList.do?queryCity=<%=key %>&queryProvince=<%=declareReportMap.get(key).get(13) %>&organizationType=<%=organizationType %>&model=<%=model %>&trainingType=<%=trainingType%>&trainingYear=<%=trainingYear%>"><%=key %></a></td>
                            										<%} else if(declareReportMap.get(key).get(0).equals(DeclareAdminConfig.ADMIN_LEVEL_REGION)){ %>
                            										<td><a href="TrainingOrganizationCountList.do?queryRegion=<%=key %>&queryProvince=<%=declareReportMap.get(key).get(13) %>&queryCity=<%=declareReportMap.get(key).get(14) %>&organizationType=<%=organizationType %>&model=<%=model %>&trainingType=<%=trainingType%>&trainingYear=<%=trainingYear%>"><%=key %></a></td>
                            										<%} else {%>
                            										<td><%=key %></td>
                            										<%}%>
