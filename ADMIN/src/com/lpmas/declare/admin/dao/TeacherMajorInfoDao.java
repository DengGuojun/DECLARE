...
public class TeacherMajorInfoDao {
	...
	public int insertMajorInfo(TeacherMajorInfoBean bean) {
		int result = -1;
	...
	public PageResultBean<TeacherMajorInfoBean> getMajorInfoPageListByMap(HashMap<String, String> condMap, PageBean pageBean) {
		...
		try {
			...
			result = dbExecutor.getPageResult(sql, orderQuery, condList, paramList, TeacherMajorInfoBean.class, pageBean, db);
		} catch (Exception e) {
			log.error("", e);
		} finally {
			try {
	...
	public List<TeacherMajorInfoBean> getMajorInfoListByMap(HashMap<String, String> condMap) {
		...
		try {
			...
			DBExecutor dbExecutor = dbFactory.getDBExecutor();
			result = dbExecutor.getRecordListResult(sql, orderQuery, condList, paramList, TeacherMajorInfoBean.class, db);
		} catch (Exception e) {
			log.error("", e);
		} finally {
			try {
