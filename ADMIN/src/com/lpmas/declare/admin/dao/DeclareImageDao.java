...
public class DeclareImageDao {
	...
	public int insertDeclareImageWithCreateTime(DeclareImageBean bean) {
		int result = -1;
		DBFactory dbFactory = new DeclareDBFactory();
		DBObject db = null;
		try {
			db = dbFactory.getDBObjectW();
			String sql = "insert into declare_image ( declare_id, image_type, image_path, status, create_time, create_user) value( ?, ?, ?, ?, ?, ?)";
			PreparedStatement ps = db.getPreparedStatement(sql);
			ps.setInt(1, bean.getDeclareId());
			ps.setString(2, bean.getImageType());
			ps.setString(3, bean.getImagePath());
			ps.setInt(4, bean.getStatus());
			ps.setTimestamp(5, bean.getCreateTime());
