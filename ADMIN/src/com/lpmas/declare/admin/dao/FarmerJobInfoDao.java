...
public class FarmerJobInfoDao {
	...
	public int insertFarmerJobInfoWtihCreateTime(FarmerJobInfoBean bean) {
		int result = -1;
		DBFactory dbFactory = new DeclareDBFactory();
		DBObject db = null;
		try {
			db = dbFactory.getDBObjectW();
			String sql = "insert into farmer_job_info (declare_id, job_type, job_id, experience, income, company_type, job_province, job_city, job_region, status, create_time, create_user, memo) value(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			PreparedStatement ps = db.getPreparedStatement(sql);
			ps.setInt(1, bean.getDeclareId());
			ps.setInt(2, bean.getJobType());
			ps.setInt(3, bean.getJobId());
			...
			ps.setString(9, bean.getJobRegion());
			ps.setInt(10, bean.getStatus());
			ps.setTimestamp(11, bean.getCreateTime());
