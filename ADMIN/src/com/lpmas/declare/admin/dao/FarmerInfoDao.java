...
public class FarmerInfoDao {
	...
	public int insertFarmerInfoWithCreateTime(FarmerInfoBean bean) {
		int result = -1;
		DBFactory dbFactory = new DeclareDBFactory();
		DBObject db = null;
		try {
			db = dbFactory.getDBObjectW();
			String sql = "insert into farmer_info (declare_id, user_name, user_gender, user_birthday, nationality, education, major, identity_number, political_status, farmer_type, status, create_time, create_user, memo) value( ?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			PreparedStatement ps = db.getPreparedStatement(sql);
			ps.setInt(1, bean.getDeclareId());
			ps.setString(2, bean.getUserName());
			ps.setInt(3, bean.getUserGender());
			...
			ps.setString(10, bean.getFarmerType());
			ps.setInt(11, bean.getStatus());
			ps.setTimestamp(12, bean.getCreateTime());
