...
public class FarmerContactInfoDao {
	...
	public int insertFarmerContactInfoWithCreateTime(FarmerContactInfoBean bean) {
		int result = -1;
		DBFactory dbFactory = new DeclareDBFactory();
		DBObject db = null;
		try {
			db = dbFactory.getDBObjectW();
			String sql = "insert into farmer_contact_info ( declare_id, user_mobile, user_email, user_qq, user_wechat, family_person, province, city, region, address, status, create_time, create_user, memo) value( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			PreparedStatement ps = db.getPreparedStatement(sql);
			ps.setInt(1, bean.getDeclareId());
			ps.setString(2, bean.getUserMobile());
			ps.setString(3, bean.getUserEmail());
