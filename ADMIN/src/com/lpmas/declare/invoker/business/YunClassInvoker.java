...
import com.lpmas.declare.invoker.bean.YunClassInvokeResponseBean;
import com.lpmas.declare.invoker.config.YunClassInvokeConfig;
import com.lpmas.declare.invoker.config.YunClassResponseStatusConfig;
				...
				if (httpResult.getResult()) {
					...
					if (responseBean.getCode() == YunClassResponseStatusConfig.STATUS_SUCCESS) {
						...
						logger.debug("第" + (count + 1) + "次请求云课堂URL:" + url.toString() + "失败,失败代码:"
								+ responseBean.getCode() + ",消息:" + responseBean.getMessage() + ",LogId:"
								+ responseBean.getLogid());
					}
				} else {
